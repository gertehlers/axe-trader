...
public final class DashboardExporter {
    ...
    public static void export(
        ...
        try {
            ...
            double maxDrawdown = maxDrawdown(trades, avgSpread);
            double worstQuarterNet = worstQuarterNet(trades, avgSpread);
    ...
    /**
     * Worst calendar-quarter total of net pnl, grouped by entry time's UTC year+quarter (so e.g.
     * 2024 Q1 and 2025 Q1 are distinct buckets). Typically negative — that's the natural reading and
     * matches the field name. Zero trades yields 0.0.
     */
    private static double worstQuarterNet(List<TradeResult> trades, double avgSpread) {
        if (trades.isEmpty()) {
            return 0.0;
        }
        Map<String, Double> byQuarter = new HashMap<>();
        for (TradeResult t : trades) {
        ...
        return byQuarter.values().stream().mapToDouble(Double::doubleValue).min().orElse(0.0);
    }
